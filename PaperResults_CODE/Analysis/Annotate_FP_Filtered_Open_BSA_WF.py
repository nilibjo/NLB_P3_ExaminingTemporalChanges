...
try:
    Zfile = "../OpenLoop_FingerPressure/ParameterVisualization/ZaoFits.csv"
...
for idx, row in data_table.iterrows():
    ...
    for filename in os.listdir('../OpenLoop_FingerPressure/ParameterVisualization/'):
        ...
        if "MultiFitsRaw_" in filename and '_'+str(patient_id) in filename: 
            ...
            Data = pd.read_csv('../OpenLoop_FingerPressure/ParameterVisualization/'+filename,header=0,delimiter=',')            
            ...
            if (len(list(Data['E_max'])) != 0) and not (np.isnan(Data_mean.mean()).any()): 
                ...
                model_out_list = [pid, partid[:-4], tday, ret_dict["P_sys"], ret_dict["P_dia"], ret_dict["SV"]/bsa, par_dict['R_sys']/bsa, par_dict['E_max']/bsa, par_dict['E_min']/bsa, par_dict['C_ao']/bsa, par_dict['Z_ao']/bsa, par_dict['t_peak'], par_dict['T'], perc_dev_p, perc_dev_q, ret_dict["SV"]]
...
Dataframe_modelestimates.to_csv('Outputs_models_FP_Open_BSA_WF.csv')
Dataframe_modelestimates.to_pickle('Outputs_models_FP_Open_BSA_WF.pkl')

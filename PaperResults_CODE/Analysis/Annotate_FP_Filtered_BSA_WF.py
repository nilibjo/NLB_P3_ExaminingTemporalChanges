...
try:
    Zfile = "../ClosedLoop_FingerPressure/ParameterVisualization/ZaoFits.csv"
    Zdata = pd.read_csv(Zfile, names=['Number', 'ID', 'partid', 'test_day', 'zval', 'eval', 'vval'], sep=',')
except Exception as exc:
    print(exc)
    print("Reference file ZaoFits file with fixed parameter values from the ClosedLoop_FingerPressure folder!")


meas_kol = ['id','partid','test_day','P_sys', 'P_dia', 'SV', 'R_sys', 'E_max', 'E_max_90perc', 'C_ao', 'data_points', 'T', 'SVubsa']
mod_kol = ['id','partid','test_day','P_sys', 'P_dia', 'SV', 'MVP', 'R_sys', 'E_max', 'C_ao', 'C_sv', 'V_tot', 'Z_ao', 't_peak', 'T', 'pWF_perc_diff', 'qWF_perc_diff', 'SVubsa']
...
for idx, row in data_table.iterrows():
    ...
    for filename in os.listdir('../ClosedLoop_FingerPressure/ParameterVisualization/'):
        ...
        if "MultiFitsRaw_" in filename and '_'+str(patient_id) in filename: 
            ...
            if (len(list(Data['E_max'])) != 0) and not (np.isnan(Data_mean.mean()).any()): 
                ...
                model_out_list = [pid, partid[:-4], tday, 
                                  ret_dict["P_sys"], ret_dict["P_dia"], ret_dict["SV"]/bsa, ret_dict['MVP'], 
                                  par_dict['R_sys']/bsa, par_dict['E_max']/bsa, par_dict['C_ao']/bsa,par_dict['C_sv']/bsa,
                                  par_dict['V_tot']/bsa, par_dict['Z_ao']/bsa, par_dict['t_peak'], 
...
Dataframe_measurements.to_csv('Outputs_measurements_FP_BSA_WF.csv')
Dataframe_measurements.to_pickle('Outputs_measurements_FP_BSA_WF.pkl')

Dataframe_modelestimates.to_csv('Outputs_models_FP_BSA_WF.csv')
Dataframe_modelestimates.to_pickle('Outputs_models_FP_BSA_WF.pkl')

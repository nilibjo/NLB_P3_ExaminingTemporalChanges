# Regression analysis - Closed-loop cardiovascular model parameter stimation results
import pandas as pd
import numpy as np
...
VO2_pre = VO2table[VO2table['test_day'] == 'Pre']
VO2_post = VO2table[VO2table['test_day'] == 'Post'] 
...
### LOOP THROUGH CAROTID MEASUREMENTS TO COMPUTE CHANGES AND COLLECT DATA
for elt in common_ids_tono:
    ...
    plot_mod_C_vec = [elt, df_mod_tono_pre[df_mod_tono_pre['partid'] == elt]['R_sys'].iloc[0], 
                           ...
                           df_mod_tono_pre[df_mod_tono_pre['partid'] == elt]['V_tot'].iloc[0], 
                           df_mod_tono_mid[df_mod_tono_mid['partid'] == elt]['V_tot'].iloc[0], 
                           df_mod_tono_post[df_mod_tono_post['partid'] == elt]['V_tot'].iloc[0],
                           df_mod_tono_pre[df_mod_tono_pre['partid'] == elt]['C_sv'].iloc[0], 
                           df_mod_tono_mid[df_mod_tono_mid['partid'] == elt]['C_sv'].iloc[0], 
                           df_mod_tono_post[df_mod_tono_post['partid'] == elt]['C_sv'].iloc[0],
...
df_reg_meas = df_meas_plot_C[["id", "SV0", "SV1", "SV2", "P0", "P1", "P2", "Pd0", "Pd1", "Pd2", "VO2_A", "VO2_B", "Sex", "Age", "delta_VO2", "wt0", "wt1", "wt2", "BMI0", "BMI1", "BMI2", "height", 'T1', 'T2', 'T3']]

# Make a selection of relevant cardiovascular model estimated data
df_reg_mod = df_mod_plot_C[["id", "R3", "R2", "R1", "C3", "C2", "C1", "E3", "E2", "E1","V1","V2","V3","Csv1","Csv2","Csv3","tp1","tp2","tp3", "Epers", "Rpers", "Cpers"]]
...
df_reg['delta_V'] = df_reg['V3'] - df_reg['V1']
df_reg['delta_Csv'] = df_reg['Csv3'] - df_reg['Csv1']
df_reg['delta_tp'] = df_reg['tp3'] - df_reg['tp1']
df_reg["delta_BMI"] = df_reg["BMI2"] - df_reg["BMI0"]

# Extra variables for regression to the mean computations
...
df_reg_std["Rpop"] = df_reg["Rpop"]
df_reg_std["Cpop"] = df_reg["Cpop"]
df_reg_std["Epop"] = df_reg["Epop"]
...
df_reg_longform3["Week"] = np.append( np.append( [0.0]*len(df_reg["id"]), [6.0]*len(df_reg["id"]) ), [12.0]*len(df_reg["id"]) )

df_reg_longform3["Rpop"] = np.append(np.append(df_reg['Rpop'], df_reg['Rpop']), df_reg['Rpop'])
df_reg_longform3["Cpop"] = np.append(np.append(df_reg['Cpop'], df_reg['Cpop']), df_reg['Cpop'])
df_reg_longform3["Epop"] = np.append(np.append(df_reg['Epop'], df_reg['Epop']), df_reg['Epop'])
df_reg_longform3["Rpers"] = np.append(np.append(df_reg['Rpers'], df_reg['Rpers']), df_reg['Rpers'])
df_reg_longform3["Cpers"] = np.append(np.append(df_reg['Cpers'], df_reg['Cpers']), df_reg['Cpers'])
df_reg_longform3["Epers"] = np.append(np.append(df_reg['Epers'], df_reg['Epers']), df_reg['Epers'])

df_reg_longform3["R1minRpop"] = np.append(np.append(df_reg['R1minRpop'], df_reg['R1minRpop']), df_reg['R1minRpop'])
df_reg_longform3["C1minCpop"] = np.append(np.append(df_reg['C1minCpop'], df_reg['C1minCpop']), df_reg['C1minCpop'])
df_reg_longform3["E1minEpop"] = np.append(np.append(df_reg['E1minEpop'], df_reg['E1minEpop']), df_reg['E1minEpop'])
df_reg_longform3["RpersRpop"] = np.append(np.append(df_reg['RpersRpop'], df_reg['RpersRpop']), df_reg['RpersRpop'])
df_reg_longform3["CpersCpop"] = np.append(np.append(df_reg['CpersCpop'], df_reg['CpersCpop']), df_reg['CpersCpop'])
df_reg_longform3["EpersEpop"] = np.append(np.append(df_reg['EpersEpop'], df_reg['EpersEpop']), df_reg['EpersEpop'])
...
df_reg_longform3_std = df_reg_longform3[["Age", "BMI", "PAI_PP", "SV", "VO2", "Week"]].select_dtypes(include=[np.number]).apply(stats.zscore,nan_policy='omit')
...
model = smf.mixedlm(formula = "Rsys ~ Age + C(Sex) + BMI", data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)

model = smf.mixedlm(formula = "Rsys ~ Age + C(Sex) + BMI + SV", missing = 'drop', data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)

model = smf.mixedlm(formula = "Rsys ~ Age + C(Sex) + BMI + VO2", missing = 'drop', data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)

model = smf.mixedlm(formula = "Rsys ~ Age + C(Sex) + BMI + SV + VO2", missing = 'drop', data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)


model = smf.ols(formula = 'delta_R ~ Age + C(Sex) + delta_BMI', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)
model = smf.ols(formula = 'delta_R ~ Age + C(Sex) + delta_BMI + delta_SV', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)
model = smf.ols(formula = 'delta_R ~ Age + C(Sex) + delta_BMI + delta_VO2', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)
model = smf.ols(formula = 'delta_R ~ Age + C(Sex) + delta_BMI + delta_VO2 + delta_SV', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)



model = smf.mixedlm(formula = "Cao ~ Age + C(Sex) + BMI", data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)

model = smf.mixedlm(formula = "Cao ~ Age + C(Sex) + BMI + SV", missing='drop', data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)

model = smf.mixedlm(formula = "Cao ~ Age + C(Sex) + BMI + VO2", missing='drop', data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)

model = smf.mixedlm(formula = "Cao ~ Age + C(Sex) + BMI + SV + VO2", missing='drop', data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)


model = smf.ols(formula = 'delta_C ~ Age + C(Sex) + delta_BMI', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)
model = smf.ols(formula = 'delta_C ~ Age + C(Sex) + delta_BMI + delta_SV', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)
model = smf.ols(formula = 'delta_C ~ Age + C(Sex) + delta_BMI + delta_VO2', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)
model = smf.ols(formula = 'delta_C ~ Age + C(Sex) + delta_BMI + delta_VO2 + delta_SV', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)



model = smf.mixedlm(formula = "Emax ~ Age + C(Sex) + BMI", data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)

model = smf.mixedlm(formula = "Emax ~ Age + C(Sex) + BMI + SV", missing='drop', data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)

model = smf.mixedlm(formula = "Emax ~ Age + C(Sex) + BMI + VO2", missing='drop', data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)

model = smf.mixedlm(formula = "Emax ~ Age + C(Sex) + BMI + SV + VO2", missing='drop', data=df_reg_longform3_std, groups=df_reg_longform3_std["id"]).fit()
print_model = model.summary()
print(print_model)


model = smf.ols(formula = 'delta_E ~ Age + C(Sex) + delta_BMI', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)
model = smf.ols(formula = 'delta_E ~ Age + C(Sex) + delta_BMI + delta_SV', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)
model = smf.ols(formula = 'delta_E ~ Age + C(Sex) + delta_BMI + delta_VO2', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)
model = smf.ols(formula = 'delta_E ~ Age + C(Sex) + delta_BMI + delta_VO2 + delta_SV', data = df_reg_std).fit()
print_model = model.summary()
print(print_model)
...
print('------------------------')
model = smf.ols(formula = 'R3 ~ RpersRpop', data = df_reg_std).fit() # *
print_model = model.summary()
print(print_model)

model = smf.ols(formula = 'delta_R ~ R1minRpop', data = df_reg_std).fit() # *
print_model = model.summary()
print(print_model)


model = smf.ols(formula = 'C3 ~ CpersCpop', data = df_reg_std).fit() # *
print_model = model.summary()
print(print_model)

model = smf.ols(formula = 'delta_C ~ C1minCpop', data = df_reg_std).fit() # *
print_model = model.summary()
print(print_model)


model = smf.ols(formula = 'E3 ~ EpersEpop', data = df_reg_std).fit() # **
print_model = model.summary()
print(print_model)

model = smf.ols(formula = 'delta_E ~ E1minEpop', data = df_reg_std).fit() # *
print_model = model.summary()
print(print_model)

...
def run_realdata_measurements(active_params,x0,writers,wflag,t_r,p_r,q_r,v_r,b_in,b_low):
    ...
    measurements = dict(P_ao=p_r,Q_lvao=q_r, SV=SVref, P_sys=Psys_ref, P_dia=Pdia_ref, MVP=6.)
    ...
    print("Standard deviation: ",standard_dev_sq)
...
if __name__ == "__main__":
    ...
    data_path = "../Data/PaperData_CPET_ReID_Compact.xlsx"
    ...
    for idx, row in data_table.iterrows():
        ...
        if np.isnan(bsa): 
            ...
            day_1 = part_tab[part_tab['test_day'] == '1CPET']
        ...
        # Estimates for total blood volume, as per the technique used by Colunga et al. (2020)
        TBV_f = (0.3562*((height/100.)**3) + 0.03308*weight + 0.1833)*1000.
        TBV_m = (0.3669*((height/100.)**3) + 0.03219*weight + 0.641)*1000.
        TBVS = (TBV_f+TBV_m)/2.*(0.13*0.27+0.64*0.18+0.035*0.5)

        print("BSA: %.2f m^2" % (bsa))
        closed_loop_base_pars["V_tot"] = TBVS
        
        print("SEVEN PARAMETER FITS-------")
        print("---------RESTING----------")
        active =         ["E_max", "C_ao", "R_sys", "V_tot", "C_sv", "t_peak", "Z_ao"]    
        x0_0 = np.array( [7.58/bsa, C_ao, R_sys, TBVS, 13.084, 0.3975, Z_ao])
        
        b_in =           [10.84/bsa, 3.0, 2.963, 1503.0,  90., np.min([0.442,T_r]), 0.2]
        b_low = np.array([0.5, 0.148, 0.917/bsa, 150.0, 1.48, np.min([0.15,0.9*T_r]), 0.001])
        ...
        writerZ.writerow([idx, row['ID'],  row['Partid'], row['test_day'],Z_ao, E_min, TBVS])
